...
#if __cplusplus >= 202002L || defined(_GLIBCXX_HAVE_BUILTIN_LAUNDER)
    ...
    struct transform
    {
        ...
        struct gen
        {
            ...
            auto operator()() -> maybe<value_type>
            {
                ...
                return ret; // I'd expect copy elision here, but
...
namespace operators
{
    ...
    ///
    /// Overloaded operator % - function application operator.
    ///
    /// Equivalent of operator `|>` in R, F#, OCaml, Elixir, Elm, Julia, Javascript...
    ///
    /// See https://www.google.com/search?q=operator+%22%7C%3E%22
    ///     https://docs.microsoft.com/en-us/dotnet/fsharp/language-reference/functions/#pipelines
    ///
    /// ```
    /// using fn::operators::operator%;  // `arg % fn1 % fn2` invokes `fn2(fn1(arg))`.
    /// using fn::operators::operator%=; // `arg %= fn1` invokes `arg = fn1(std::move(arg))`
    /// ```
    ///
    /// Why `operator %` ?
    ...
    ///
    /// Also, in `std::ranges` and `range-v3` `operator |` is used for composition of views
