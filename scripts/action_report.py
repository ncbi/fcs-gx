...
-----------------------------------------------------------------------------
"""
# pylint: disable=C0301,C0114,C0103,C0116,C0115,R0914,R0916,R0911,W0511,W0603,C0302
#############################################################################
# pylint: disable=C0301,C0302,C0114,C0103,C0116,C0115,R0914,R0916,R0911
# fmt: off
...
def dsprint(seq_id, *args, **kwargs):
    if global_debug or (global_debug_seq_id and (seq_id == global_debug_seq_id or seq_id.startswith(global_debug_seq_id+"~"))):
        print(*args, file=sys.stderr, **kwargs)
...
def can_host_virus(div, vir_div) -> bool:
    if div[:5] == "virs:" or vir_div[:5] != "virs:":
        return False
    if is_prok(div):
        if vir_div.startswith("virs:prokaryotic"):
            return True
    else:
        if vir_div.startswith("virs:eukaryotic"):
            return True
    return False
...
def is_mostly_review_contam(records: List) -> bool:
    ...
    for r in records:
        #dprint(f"  {r.result} ")
        record_len = ((r.end_pos-r.start_pos)+1)
        if is_contam(r.result):
            if r.result == CONTAM_LOW_COV:
                review_contam_len += record_len
            else:
                normal_contam_len += record_len
    #dprint(f"normal_contam_len: {normal_contam_len} , review_contam_len: {review_contam_len}")
    return (review_contam_len >= normal_contam_len)
...
result_labels_contam.add(CONTAM_LOW_COV)
...
def result_group(s) -> int:
    # 1 - contam, 2 - primary, -1 - inconclusive
    return ResultGroup.CONTAM if is_contam(s) else ResultGroup.PRIMARY if is_primary(s) else ResultGroup.INCONCLUSIVE
...
class Record:
    ...
    @staticmethod
    def id_to_range(seq_id: str) -> dict:
        tilda_count = seq_id.count("~")

        if tilda_count == 0:
            return {"id": seq_id, "start_pos": 0, "end_pos": 0, "whole": True, "gaps": [True, True]}
        elif tilda_count in (1, 2):  # LR994621.1~46904608..46976173
            i1 = seq_id.split("~")
            i2 = i1[-1].split("..")
            return {"id": i1[0], "start_pos": int(i2[0]), "end_pos": int(i2[1]), "whole": False, "gaps": [True, True]}
        elif tilda_count == 3:  # LR994621.1~20258968..20296541~~33292..35019
            ...
            if i3[0] == "1":
                eg = True
            if int(i3[1]) == total_len:
                lg = True
            return {"id": i1[0], "start_pos": start_pos, "end_pos": end_pos, "whole": False, "gaps": [eg, lg]}
        return {}
    ...
    def from_row(row: List[str]):
        ...
        if "~" in rec.seq_id:
            ...
            rec.start_pos = row_range["start_pos"]
            rec.end_pos = row_range["end_pos"]
...
class Span:
    ...
    def __init__(self, records: List[Record], result: str, orig_id: str, start_pos: int, end_pos: int, whole: bool):
    ...
    def make_span(records: List[Record]):
        ...
        span = Span(
            records, records[0].result, records[0].orig_id, records[0].start_pos, records[-1].end_pos, records[0].whole
        )
        span.rules = []
    ...
    @staticmethod
    def grey_side(p: Record, r: Record):
    ...
    def calc_spans(records: List[Record]):
        ...
        big_primary_spans = []
        for s in spans:
            if s.primary_len >= 50000:
                big_primary_spans.append(c)
            c = c + 1
        ...
        if print_rules:
            return "\t".join(
                [
                    ...
                    str(self.agg_cov),
                    self.top_tax_name,
                    ":".join([*self.span.rules, self.rule]),
                ]
            )
        return "\t".join(
            [
...
re_endosymbiont = re.compile(r"wolbachia|rickettsia", re.I)
...
class Sequence:
    ...
    def range_has_gaps(self, left, right):
        for j in range(left, right):
            ...
            if first.end_pos < second.start_pos - 1:
                if ((first.result == "primary-div" and is_contam(second.result))
        ...
        for i in range(len(primary_div_indices)-1):
            ...
            for j in range(left_flank+1, right_flank):
                ...
                if s.result == "inconclusive":
                    continue
                if re_endosymbiont.search(s.top_tax_name) and not is_prok(left_flank_div):
                    # Valid endosymbiont
    ...
    def calc_actions(self, primary_div_name: str, low_cov_mode: bool, pa_same_kingdom_contam_demote: bool, is_same_kingdom_demote: bool, thr: ExcludeThresholds):
        spans = self.spans
        ...
        if len(spans) == 0:
            ...
            if is_primary(s.result):
                ...
                # elif not is_prok(primary_div_name) and re_endosymbiont.search(s.top_tax_name):
                #     action_str = "INFO"
                #     rule = "Endosymbiont"
            ...
            if global_debug or action_str in ("EXCLUDE", "REVIEW", "REVIEW_RARE", "INFO"):
            ...
            if (keep_len < thr.good_abs_len) or (
                ...
                if pct_cvg <= global_low_coverage_threshold:
                    ...
                    action_str = "EXCLUDE"
                    action_rule = "M01"
                    if (is_same_kingdom(primary_div_name, seq_stat.max_div_name) and seq_stat.max_div_cov < 20):
                        action_str = "REVIEW"
                ...
                for i, s in enumerate(spans):
                    ...
                    if is_primary(s.result):
                        ...
                        # elif ...
                        if not is_prok(primary_div_name) and re_endosymbiont.search(s.top_tax_name):
                            trim_action = "INFO"
                            ...
                            if is_same_kingdom(primary_div_name, s.max_div_name):
        ...
        for span in seq.spans:
            for r in span.records:
                ...
                if result_group(r.result) != -1:
                    ...
                    seq_divs.add(r.div)
                if result_group(r.result) != -1 or r.result == "low-coverage":
...
def fix_action(action: Action, prefix: str) -> Action:
    ...
    return Action(action.seq_id, action.start_pos, action.end_pos, action.len, action_str,
                  action.div, action.agg_cov, action.top_tax_name, action.span, prefix + action.rule)
...
def no_review_without_excludes_rule(seqs: List):
    ...
    divs_with_exclude = set((a.div for seq in seqs for a in seq.actions if a.action == "EXCLUDE"))
    if global_debug:
        for seq in seqs:
            seq.actions = [a if a.action != "REVIEW" or a.div in divs_with_exclude else fix_action(a, "No EXCLUDE for div, ") for a in seq.actions ]
    else:
        for seq in seqs:
            seq.actions = [a for a in seq.actions if a.action != "REVIEW" or a.div in divs_with_exclude]
...
def action_report(args):
    ...
    for seq in seqs:
        seq.calc_actions(primary_div_name, low_cov_mode, pa_same_kingdom_contam_demote, args.ignore_same_kingdom, thresholds)
